...
import com.eventmanagement.bookingservice.messaging.EventPublisher;
...
public class BookingService {
    ...
    public BookingResponse bookTicket(BookingRequest request) {
        ...
        // Publish event to RabbitMQ instead of direct API call
        try {
            TicketBookedEvent ticketEvent = TicketBookedEvent.builder()
                    .ticketId(savedTicket.getId())
                    .eventId(savedTicket.getEventId())
                    .attendeeId(savedTicket.getAttendeeId())
                    .attendeeEmail(savedTicket.getAttendeeEmail())
                    .price(savedTicket.getPrice())
                    .status(savedTicket.getStatus().toString())
                    .qrCode(savedTicket.getQrCode())
                    .bookingDate(savedTicket.getBookingDate())
                    .build();

            eventPublisher.publishTicketBookedEvent(ticketEvent);
            eventPublisher.publishEmailNotification(ticketEvent);

            System.out.println("Booking Service: Events published to RabbitMQ");
        } catch (Exception e) {
            System.err.println("Booking Service: Failed to publish events: " + e.getMessage());
        }
